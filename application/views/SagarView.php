...
<body>
      ...
      <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse navbar-ex1-collapse">
          ...
          <form class="navbar-form navbar-left" method="POST" role="search"
            action="<?php echo site_url('Sagarcontroller/getSingle') ?>">
  ...
  <main style="margin-top:80px; ">
    <div class="container">
      <!-- Toast placed at top --> <?php if ($this->session->flashdata('success')): ?>
        <div class="toast-top">
          <strong>Action Performed</strong><br> <?= $this->session->flashdata('success'); ?>
        </div> <?php endif; ?>

      <?php if ($errors = $this->session->flashdata('csv_errors')): ?>
        <?php foreach ($errors as $err): ?>
          <div class="toast-top error-toast">
            <strong>Skipped Entry</strong><br> <?= $err ?>
          </div>
        <?php endforeach; ?>
      <?php endif; ?>
            ...
            <thead>
              <tr>
                ...
                <th> <button type="button" id="deleteAllBtn" class="btn btn-danger btn-xs"> Delete All </button>
                  <button type="button" id="deleteSelectedBtn" class="btn btn-danger btn-xs" style="display:none;">
                ...
                <tr>
                  ...
                  <td class="text-center">
                    <a data-toggle="modal" href='#modal-id' data-id="<?= $contact['id'] ?>"
                      data-contactno="<?= $contact['contactno'] ?>" data-contactname="<?= $contact['contactname'] ?>"
                      data-address="<?= $contact['address'] ?>" class="btn btn-xs btn-warning editBtn">Edit</a>
                    <a href="<?= site_url('Sagarcontroller/delete/' . $contact['id']) ?>"
  ...
  <script>
    $(document).ready(function () {
      $(".toast-top").each(function (i) {
        let toast = $(this);

        setTimeout(() => {
          toast.fadeIn(400).delay(3000).fadeOut(400);
        }, i * 500); // stagger so they don't overlap badly
      });
    });
  ...
  <script>
    $(document).ready(function () {
      ...
      <?php if ($this->session->flashdata('edit_errors')): ?>
        ...
        // Fill modal inputs with previously submitted values
        $("#contact_id").val("<?= $this->session->flashdata('edit_id') ?>");
        $("#contact_no").val("<?= $this->session->flashdata('edit_contactno') ?>");
        $("#contact_name").val("<?= $this->session->flashdata('edit_contactname') ?>");
        $("#contact_address").val("<?= $this->session->flashdata('edit_address') ?>");
